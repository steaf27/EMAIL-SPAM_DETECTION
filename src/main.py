...
def main():
    ...
    print(
        f"  - Training set: {X_train.shape[0]} ({X_train.shape[0] / len(original_df) * 100:.1f}%)"
    )
    print(
        f"  - Testing set: {X_test.shape[0]} ({X_test.shape[0] / len(original_df) * 100:.1f}%)"
    )
    print(f"  - Feature dimensionality: {X_train.shape[1]}")
    print(
        f"  - Distribusi label (Ham:Spam): {sum(y_train == 0)}:{sum(y_train == 1)} (train)"
    )
    ...
    top_features, top_importances = get_feature_importance(
        best_model, vectorizer, top_n=20
    )
